...
import { LOCALITIES, PARAMS, generateReport } from "@/lib/mock-data";
...
function ComparePage() {
  ...
  const initial = [sp.loc1, sp.loc2, sp.loc3].filter(Boolean);
  const [ids, setIds] = useState(initial.length ? initial : ["koramangala", "indiranagar"]);

  const reports = useMemo(
    () => ids.map((id) => generateReport(LOCALITIES.find((l) => l.id === id))),
    [ids],
  );

  const setAt = (i, id) => setIds(ids.map((x, k) => (k === i ? id : x)));
  const removeAt = (i) => setIds(ids.filter((_, k) => k !== i));
  const add = () => {
    ...
    <PageShell>
      <div className="mx-auto max-w-7xl px-4 py-10 md:px-6">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-3">
          ...
          <Button variant="outline" onClick={csv} className="gap-2">
            ...
            <table className="w-full text-sm">
              <thead className="border-b bg-muted/40">
                <tr>
                  ...
                  {reports.map((r) => (
                    <th key={r.locality.id} className="px-4 py-3 text-left font-medium">
                      {r.locality.name}
                    </th>
                  ))}
                </tr>
